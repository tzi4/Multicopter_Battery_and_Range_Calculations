"""Compare an optional, trusted local range-calculator archive with this package.
...
def load_archive(module_path):
    # Imports execute Python: use only your own trusted source archive.
    spec = importlib.util.spec_from_file_location("local_range_archive", module_path)
...
def build_suite(module, data_root, attitude_csv, calculator_name, *, diameter, correction_factor):
    ...
    profile["attitude_log_csv"] = attitude_csv
    energy_wh = module.calculate_real_energy_wh(12, 27000, "liion")
    calculator = getattr(module, calculator_name)
...
def input_comparison(archive_data_root, archive_attitude_csv):
    ...
    for path in sorted(public.rglob("*")):
        ...
        original = archive_attitude_csv if relative.as_posix() == "flight_attitude.csv" else archive_data_root / relative
...
def compare(archive_root, *, module_path, data_root, attitude_csv, calculator_name):
    original_hash = sha256(module_path)
    archive = load_archive(module_path)
    ...
    for diameter in (28.0, 29.0):
        ...
        old_solved, old_suite = build_suite(archive, data_root, attitude_csv, calculator_name, diameter=diameter, correction_factor=cf_old["correction_factor"])
        public_data = ROOT / "data/calibration/2026-07-03"
        new_solved, new_suite = build_suite(current, public_data, public_data / "flight_attitude.csv", "BauersfeldRangeCalculator", diameter=diameter, correction_factor=cf_old["correction_factor"])
        if diameter == 28:
    ...
    inputs = input_comparison(data_root, attitude_csv)
    result = {"schema_version": 1, "scope": "Same-input code comparison, not independent flight validation.",
              "archive_module_description": "Original local range calculator", "archive_module_sha256": original_hash,
              ...
              "archive_module_unchanged": sha256(module_path) == original_hash}
...
def main():
    ...
    parser.add_argument("archive_root", type=Path, help="Trusted local directory containing the original calculator, July 3 inputs and saved report")
    parser.add_argument("--module", type=Path, default=Path("archived_range.py"), help="Calculator source path, absolute or relative to archive_root")
    parser.add_argument("--data-root", type=Path, default=Path("data/calibration/2026-07-03"), help="July 3 telemetry directory, absolute or relative to archive_root")
    parser.add_argument("--attitude-csv", type=Path, default=Path("flight_attitude_00000075.csv"), help="Attitude CSV path, absolute or relative to archive_root")
    parser.add_argument("--calculator-class", default="BauersfeldRangeCalculator", help="Name of the Bauersfeld calculator class in the supplied module")
    parser.add_argument("--output", type=Path, default=ROOT / "docs/results/current-local-comparison.json")
    args = parser.parse_args()
    archive_root = args.archive_root.resolve()
    result = compare(archive_root, module_path=archive_root / args.module,
                     data_root=archive_root / args.data_root,
                     attitude_csv=archive_root / args.attitude_csv,
                     calculator_name=args.calculator_class)
