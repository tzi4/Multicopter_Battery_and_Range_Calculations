...
executes the author's original range calculator and the current package with the
...
python examples/compare_local_archive.py /path/to/original/archive \
  --module archived_range.py \
  --data-root data/calibration/2026-07-03 \
  --calculator-class BauersfeldRangeCalculator
```

The command above shows the default source path, telemetry directory and
calculator class. Set these options to the names in your own archive. Relative
paths are resolved under the archive directory, and absolute paths are also
accepted. Use `--attitude-csv` if its attitude file has a different path from
`flight_attitude_00000075.csv`. The archive directory must also contain
`datalink_fit_method_report.md`. The script imports the supplied Python file,
so use your own trusted archive. It writes
